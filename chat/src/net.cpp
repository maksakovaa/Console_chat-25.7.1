...
net::net()
{
    ...
    if (iResult != 0)
    {
        cout << "[" << curDateTime() << "] ERROR: Ошибка WSAStartup: " << iResult << endl;
        exit(1);
    }
    ...
    if (iResult != 0)
    {
        cout << "[" << curDateTime()<< "] ERROR: Ошибка getaddrinfo: " << iResult << endl;
        WSACleanup();
        exit(1);
    }
    ...
    for (ptr = result; ptr != NULL; ptr = ptr->ai_next)
    {
        ...
        {
            cout << "[" << curDateTime() << "] ERROR: Ошибка создания сокета: " << WSAGetLastError() << endl;
            WSACleanup();
            exit(1);
        }
...
void net::readmsg()
{
    ...
    if (iResult == 0)
    {
        cout << "[" << curDateTime() << "] ERROR: Соединение закрыто." << endl;
        exit(1);
    }
    else if (iResult < 0)
    {
        cout << "[" << curDateTime() << "] ERROR: Ошибка получения: " << WSAGetLastError() << endl;
        exit(1);
    }
#elif defined (__linux__)
...
void net::getMsgBase(std::vector<string> *sMsg)
{
    ...
    while (strncmp("MSGBASE_END", package, sizeof("MSGBASE_END") - 1) != 0)
	{
        ...
        {
            cout << "[" << "] База сообщений обновлена." << endl;
            break;
        }
        else
        {
